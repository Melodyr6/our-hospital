...
public class CreportServiceImpl implements CreportService {
    ...
    @Override
    public Integer selDocReport(PaiBanVo paiBanVo){return creportMapper.selDocReport(paiBanVo); }
    ...
    public Integer addre(CReport cReport) {
        if (StringUtils.isEmpty(cReport.getTime())) {
            cReport.setTime(new Date().toString());
        }
